...
from make_prg.utils.seq_utils import Sequence, SequenceExpander, Sequences, ungap
...
def cluster_further(clusters: ClusteredSeqs) -> bool:
    def not_ref_like(sequences):
        return not sequences_are_one_reference_like(sequences)
...
def extract_clusters(
    ...
    if num_elems != len(value_pool):
        raise ValueError(
            "Mismatch between number of sequences/ID lists and number of cluster "
            "assignments"
        )
    num_clusters = max(cluster_assignment) + 1
    if set(cluster_assignment) != set(range(num_clusters)):
        raise ValueError(
            "Inconsistent cluster numbering (likely reason: more input sequences that "
...
def merge_sequences(*seqlists: Sequences, first_seq: str) -> Sequences:
    ...
    assert first_seq_found, (
        f"Provided first sequence argument ({first_seq}) not found in provided list "
        "of sequences "
        f"({seqlists})"
    )

    merged_unexpanded_sequences = [first_seq] + result
    merged_expanded_sequences = SequenceExpander.get_expanded_sequences(
        merged_unexpanded_sequences
    )
    ...
    if first_seq_has_ambiguous_char:
        logger.warning(
            f"Provided first sequence argument ({first_seq}) has an ambiguous base, "
            "and thus does not match expanded first sequence "
            f"({first_merged_expanded_sequence})"
        )
...
def kmeans_cluster_seqs(
    ...
    """
    logger.debug(
        f"kmeans_cluster_seqs: Starting clustering with {len(alignment)} sequences, kmer_size={kmer_size}"
    )
    ...
    logger.debug("kmeans_cluster_seqs: Processing sequences for clustering")
    ...
    if too_few_seqs_to_cluster:
        logger.debug(
            f"kmeans_cluster_seqs: Too few sequences to cluster ({num_sequences}), returning single cluster"
        )
        single_cluster = [
    ...
    logger.debug(
        f"kmeans_cluster_seqs: Built count matrix of shape {count_matrix.shape}"
    )
    ...
    logger.debug("kmeans_cluster_seqs: Starting clustering loop")
    while cluster_further(seqclustering):
        ...
        if num_clusters > MAX_CLUSTERS:
            logger.debug(
                f"kmeans_cluster_seqs: Reached max clusters limit ({MAX_CLUSTERS})"
            )
            break
        if num_clusters == num_sequences:
            logger.debug(
                "kmeans_cluster_seqs: Number of clusters equals number of sequences"
            )
            break

        logger.debug(
            f"kmeans_cluster_seqs: Running KMeans with {num_clusters} clusters"
        )
        kmeans = KMeans(n_clusters=num_clusters, random_state=2, algorithm="elkan").fit(
            count_matrix
        )
        logger.debug("kmeans_cluster_seqs: KMeans fitting completed")
        ...
        if num_fitted_clusters < num_clusters:
            logger.debug(
                "kmeans_cluster_seqs: Fitted clusters < requested clusters, reverting"
            )
        ...
        logger.debug(
            f"kmeans_cluster_seqs: Extracted {len(seqclustering)} sequence clusters"
        )

    no_clustering = num_clusters == 1 or num_clusters == num_sequences
    logger.debug(
        f"kmeans_cluster_seqs: Final clustering result - {num_clusters} clusters, no_clustering={no_clustering}"
    )

    if no_clustering:
        logger.debug(
            "kmeans_cluster_seqs: No meaningful clustering found, returning single cluster"
        )
        single_cluster = [
            flatten_list(seq_to_ids.values()) + flatten_list(small_seq_to_ids.values())
        ]
        ...
        logger.debug(
            f"kmeans_cluster_seqs: Meaningful clustering found, creating {num_clusters} clusters"
        )
        clustered_ids: ClusteredIDs = []
        if num_sequences > 0:
            clustered_ids = extract_clusters(seq_to_ids, cluster_assignment)
        clustered_ids = merge_clusters(
            clustered_ids, small_seq_to_ids.values(), first_id=first_id
        )
