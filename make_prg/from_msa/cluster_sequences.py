...
def kmeans_cluster_seqs(
    ...
    while cluster_further(seqclustering):
        ...
        if num_clusters == num_sequences:
            logger.debug(
                f"kmeans_cluster_seqs: Number of clusters equals number of sequences"
            )
            break
        ...
        if num_fitted_clusters < num_clusters:
            logger.debug(
                f"kmeans_cluster_seqs: Fitted clusters < requested clusters, reverting"
            )
    ...
    if no_clustering:
        logger.debug(
            f"kmeans_cluster_seqs: No meaningful clustering found, returning single cluster"
        )
        single_cluster = [
            flatten_list(seq_to_ids.values()) + flatten_list(small_seq_to_ids.values())
        ]
