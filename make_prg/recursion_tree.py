...
class RecursiveTreeNode(ABC):
    ...
    def __init__(
        ...
        logger.debug(
            f"Creating node {self._node_id} ({parent_info}, nesting={nesting_level}, {len(children_subalignments)} subalignments)"
        )
        ...
        logger.debug(
            f"Node {self._node_id} construction completed with {len(self._children)} children"
        )
    ...
    def _get_children(
        ...
        logger.debug(
            f"Node {self.node_id}: Building {len(children_subalignments)} child nodes"
        )
        children = []
        for i, alignment in enumerate(children_subalignments):
            logger.debug(
                f"Node {self.node_id}: Building child {i+1}/{len(children_subalignments)} with {len(alignment)} sequences"
            )
            child = NodeFactory.build(alignment, self.prg_builder, self)
            children.append(child)
            logger.debug(
                f"Node {self.node_id}: Completed child {i+1}/{len(children_subalignments)}"
            )
        logger.debug(
            f"Node {self.node_id}: All {len(children)} children built successfully"
        )
        return children
...
class LeafNode(RecursiveTreeNode):
    ...
    def preorder_traversal_to_build_prg(
        ...
        expanded_sequences = SequenceExpander.get_expanded_sequences_from_MSA(
            self.alignment,
            force_expand_ambiguous_bases=self.prg_builder.force_expand_ambiguous_bases,
        )
...
class NodeFactory:
    ...
    def build(
        ...
        logger.debug(
            f"NodeFactory.build: Starting to build node (root={building_the_root}, nesting_level={nesting_level})"
        )
        logger.debug(
            f"NodeFactory.build: Alignment has {len(alignment)} sequences, length={alignment.get_alignment_length()}"
        )

        min_match_length = prg_builder.min_match_length
        logger.debug(
            f"NodeFactory.build: Getting vertical partition with min_match_length={min_match_length}"
        )
        all_intervals, match_intervals = NodeFactory._get_vertical_partition(
            alignment, min_match_length, prg_builder
        )
        logger.debug(
            f"NodeFactory.build: Found {len(all_intervals)} intervals, {len(match_intervals)} match intervals"
        )
        ...
        logger.debug(
            f"NodeFactory.build: building_a_leaf={building_a_leaf}, building_multi_interval_node={building_multi_interval_node}"
        )
        ...
        if building_a_leaf:
            ...
            logger.debug(
                f"NodeFactory.build: Created {len(interval_subalignments)} interval subalignments"
            )
            ...
            if cluster_further:
                logger.debug(
                    f"NodeFactory.build: Building multi-cluster node, increasing nesting level to {nesting_level + 1}"
                )
                ...
                logger.debug(
                    f"NodeFactory.build: Created {len(cluster_subalignments)} cluster subalignments"
                )
    ...
    def _get_vertical_partition(
        ...
        logger.debug(
            f"_get_vertical_partition: Intervals computed - {len(all_intervals)} total, {len(match_intervals)} matches"
        )
