...
class InputOutputFiles(ABC):
    ...
    """
    def __init__(self, locus_name: str, output_type: OutputType, root_temp_dir: Path, output_stats: bool):
    ...
    def create_final_files(list_of_InputOutputFiles: List["InputOutputFiles"], output_prefix: str):
        ...
        if output_type.prg:
            ...
            prg_files = [input_output_file.prg for input_output_file in list_of_InputOutputFiles]
            prg_files = sorted(prg_files)  # guarantees reproducibility
            InputOutputFiles.concatenate_text_files(prg_files, output_prefix + ".prg.fa")

            # zip all PRG Builders
            logger.info("Creating update data structures...")
            prg_builder_zip_db = PrgBuilderZipDatabase(Path(f"{output_prefix}.update_DS.zip"))
            locus_to_prg_builder_pickle_path = {input_output_file.locus_name: input_output_file.pickle
        ...
        if output_type.binary:
            ...
            filename_to_encoded_PRG_paths = {input_output_file.bin.name: input_output_file.bin
                                                for input_output_file in list_of_InputOutputFiles}
        ...
        if output_type.gfa:
            ...
            filename_to_gfa_paths = {input_output_file.gfa.name: input_output_file.gfa
                                                for input_output_file in list_of_InputOutputFiles}
            if is_a_single_MSA:
        ...
        if output_stats:
            ...
            stats_files = [input_output_file.stats for input_output_file in list_of_InputOutputFiles]
            (
...
class InputOutputFilesFromMSA(InputOutputFiles):
    ...
    @staticmethod
    def get_list_of_InputOutputFilesFromMSA(input_files: List[Path], output_type: OutputType, root_temp_dir: Path) \
...
class InputOutputFilesUpdate(InputOutputFiles):
    ...
    @staticmethod
    def get_list_of_InputOutputFilesUpdate(loci: List[str], output_type: OutputType, root_temp_dir: Path) \
            -> List["InputOutputFilesUpdate"]:
        return [InputOutputFilesUpdate(locus, output_type, root_temp_dir) for locus in loci]
