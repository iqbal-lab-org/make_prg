...
from make_prg.from_msa.interval_partition import Interval, IntervalType
...
from make_prg.update.denovo_variants import MLPathError, UpdateData
...
@patch(
    "make_prg.recursion_tree.remove_columns_full_of_gaps_from_MSA",
...
class TestRecursiveTreeNode(TestCase):
    ...
    def test___eq___different_nesting_level(self, *uninteresting_mocks):
        self.setup()
    ...
    def test___eq___different_locus_name(self, *uninteresting_mocks):
        self.setup()
        node_1 = MultiIntervalNode(
            nesting_level=1,
            alignment=self.alignment_mock,
        ...
        node_2 = MultiIntervalNode(
            ...
            alignment=self.alignment_mock,
            parent=self.parent_mock,
            prg_builder=PrgBuilder("other_locus", Path("msa"), "fasta", 5, 7),
            interval_subalignments=["algn1"],
        )
        self.assertNotEqual(node_1, node_2)
    ...
    def test___eq___different_node_id(self, *uninteresting_mocks):
        self.setup()
        node_1 = MultiIntervalNode(
            nesting_level=1,
            alignment=self.alignment_mock,
    ...
    def test____get_children___one_child(
        self, node_factory_build_mock, *uninteresting_mocks
    ):
        self.setup()
        node = MultiIntervalNode(
            1,
            self.alignment_mock,
            self.parent_mock,
            self.prg_builder,
    ...
    @patch.object(
        NodeFactory,
        NodeFactory.build.__name__,
        side_effect=["child_build_1", "child_build_2", "child_build_3"],
    )
    def test____get_children___three_children(
        self, node_factory_build_mock, *uninteresting_mocks
    ):
        self.setup()
        node = MultiIntervalNode(
        ...
        self.assertEqual(
            ["child_build_1", "child_build_2", "child_build_3"], node.children
        )

    @patch.object(
        MultiIntervalNode,
        MultiIntervalNode._get_children.__name__,
        return_value=["child"],
    )
    ...
    def test___is_root___is_not_root(self, *uninteresting_mocks):
        self.setup()
        node = MultiIntervalNode(
            1, self.alignment_mock, self.parent_mock, self.prg_builder, []
        )
    ...
    def test___replace_child___old_child_not_present___raises_Assertion_Error(
        self, *uninteresting_mocks
    ):
        self.setup()
        node = MultiIntervalNode(
...
class TestMultiIntervalNode(TestCase):
    ...
    @patch.object(
        MultiIntervalNode,
        MultiIntervalNode._get_children.__name__,
        return_value=["child_1", "child_2"],
    )
    def test___constructor(
        self,
        get_children_mock,
        remove_columns_full_of_gaps_from_MSA_mock,
        *uninteresting_mocks,
    ):
        self.setup()
        children_mock = Mock()
        node = MultiIntervalNode(
            1, self.alignment_mock, self.parent_mock, self.prg_builder, children_mock
        )

        self.assertEqual(1, node.nesting_level)
        remove_columns_full_of_gaps_from_MSA_mock.assert_called_once_with(
            self.alignment_mock
        )
    ...
    @patch.object(
        MultiIntervalNode, MultiIntervalNode._get_children.__name__, return_value=[]
    )
    def test___constructor___no_children___raises_AssertionError(
        self, *uninteresting_mocks
    ):
        self.setup()
        children_mock = Mock()
    ...
    def test___preorder_traversal_to_build_prg___single_child(
        self, *uninteresting_mocks
    ):
        self.setup()
        children_mock = Mock()
    ...
    @patch.object(
        ...
        MultiIntervalNode._get_children.__name__,
        return_value=[
            Preorder_traversal_to_build_prg_Mock(1),
            Preorder_traversal_to_build_prg_Mock(2),
            Preorder_traversal_to_build_prg_Mock(3),
        ],
    )
    def test___preorder_traversal_to_build_prg___multiple_children(
        self, *uninteresting_mocks
    ):
        self.setup()
        children_mock = Mock()
...
class TestMultiClusterNode(TestCase):
    ...
    @patch.object(
        MultiClusterNode,
        MultiClusterNode._get_children.__name__,
        return_value=["child_1", "child_2"],
    )
    def test___constructor(
        self,
    ...
    @patch.object(
        MultiClusterNode, MultiClusterNode._get_children.__name__, return_value=[]
    )
    def test___constructor___no_children___raises_AssertionError(
        self, *uninteresting_mocks
    ):
...
class TestLeafNode(TestCase):
    ...
    @patch.object(
        SequenceExpander,
        SequenceExpander.get_expanded_sequences_from_MSA.__name__,
        return_value=["ACGT"],
    )
    @patch.object(PrgBuilder, PrgBuilder.update_PRG_index.__name__)
    def test___preorder_traversal_to_build_prg___single_seq(
        self, update_prg_index_mock, *uninteresting_mocks
    ):
        self.setup()
    ...
    @patch.object(PrgBuilder, PrgBuilder.update_PRG_index.__name__)
    def test___preorder_traversal_to_build_prg___single_seq___no_indexing(
        self, update_prg_index_mock, *uninteresting_mocks
    ):
        self.setup()
        ...
        expected_prg_as_list = list("ACGT")
        actual_prg_as_list = []
        self.node.preorder_traversal_to_build_prg(
            actual_prg_as_list, delim_char="*", do_indexing=False
        )
    ...
    @patch.object(
        SequenceExpander,
        SequenceExpander.get_expanded_sequences_from_MSA.__name__,
        return_value=["AA", "C", "GGGG"],
    )
    @patch.object(PrgBuilder, PrgBuilder.get_next_site_num.__name__, return_value=42)
    @patch.object(PrgBuilder, PrgBuilder.update_PRG_index.__name__)
    def test___preorder_traversal_to_build_prg___several_seqs(
        self, update_prg_index_mock, *uninteresting_mocks
    ):
        self.setup()
    ...
    @patch.object(PrgBuilder, PrgBuilder.get_next_site_num.__name__, return_value=42)
    @patch.object(PrgBuilder, PrgBuilder.update_PRG_index.__name__)
    def test___preorder_traversal_to_build_prg___several_seqs___no_indexing(
        self, update_prg_index_mock, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___add_data_to_batch_update___interval_not_indexed___raises_UpdateError(
        self, *uninteresting_mocks
    ):
        self.setup()
        ml_path_mock = Mock()
    ...
    def test___add_data_to_batch_update___update_data_interval_flanked(
        self, *uninteresting_mocks
    ):
        self.setup()
        ...
        get_node_given_interval_in_PRG_space_mock = Mock(
            side_effect=get_node_given_interval_in_PRG_space_mock_fun
        )
        ml_path_node_mock = Mock(
            get_node_given_interval_in_PRG_space=get_node_given_interval_in_PRG_space_mock
        )
    ...
    def test___add_data_to_batch_update___update_data_interval_on_the_left_end(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___add_data_to_batch_update___update_data_interval_on_the_right_end(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___add_data_to_batch_update___update_data_interval_flanked___left_interval_not_in_ml_path(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___add_data_to_batch_update___update_data_interval_flanked___right_interval_not_in_ml_path(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___add_indexed_PRG_interval___multiple_intervals(
        self, *uninteresting_mocks
    ):
        self.setup()
        ...
        self.assertEqual(
            {(0, 4), (10, 15), (17, 19), (25, 30)}, self.node.indexed_PRG_intervals
        )

    @patch.object(LeafNode, LeafNode._update_leaf.__name__)
    def test___batch_update___no_new_sequences___no_update_to_be_done(
        self, update_leaf_mock, *uninteresting_mocks
    ):
        self.setup()
    ...
    @patch.object(LeafNode, LeafNode._update_leaf.__name__)
    def test___batch_update___one_new_sequence___update_is_called(
        self, update_leaf_mock, *uninteresting_mocks
    ):
        self.setup()
    ...
    def test___update_leaf___no_aligner_was_given___raises_AssertionError(
        self, *uninteresting_mocks
    ):
        self.setup()
        self.node.new_sequences.add("ACGT")
    ...
    def test___update_leaf___is_not_root(self, *uninteresting_mocks):
        ...
        updated_child = Mock()
        with patch.object(
            NodeFactory, "build", return_value=updated_child
        ) as node_factory_build_mock:
            self.node._update_leaf()
...
class TestNodeFactory(TestCase):
    ...
    @patch.object(
        NodeFactory, NodeFactory._is_single_match_interval.__name__, return_value=True
    )
    def test___build___parent_node_is_None___is_a_single_match_interval___builds_leaf_root(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    @patch.object(
        NodeFactory, NodeFactory._is_multi_interval.__name__, return_value=True
    )
    @patch.object(
    ...
    def test___build___is_a_single_match_interval_and_non_root___builds_leaf(
        self, *uninteresting_mocks
    ):
        self.setup()
        ...
        with patch.object(LeafNode, "__init__", __init__):
            node = NodeFactory.build(
                self.alignment, self.prg_builder, parent_node=outer_parent
            )
            self.assertTrue(isinstance(node, LeafNode))
    ...
    def test___build___has_multiple_intervals___builds_multi_interval_non_root_node(
        self, *uninteresting_mocks
    ):
        self.setup()
        ...
        # mock MultiIntervalNode.__init__
        def __MultiIntervalNode_init__(
            node_self,
            nesting_level,
            alignment,
            parent,
    ...
    @patch.object(
        NodeFactory,
        NodeFactory._infer_if_we_should_cluster_further.__name__,
        return_value=True,
    )
    @patch.object(
        NodeFactory,
        NodeFactory._get_subalignments_by_clustering.__name__,
        return_value="get_subalignments_by_clustering_mock",
    )
    def test___build___has_single_interval_but_multiple_clusters___builds_multi_cluster_non_root_node(
        self, *uninteresting_mocks
    ):
        self.setup()
        ...
        def __MultiClusterNode_init__(
            ...
            self.assertEqual(id(self.prg_builder), id(prg_builder))
            self.assertEqual(
                "get_subalignments_by_clustering_mock", cluster_subalignments
            )
    ...
    def test___build___has_single_interval_but_cant_cluster_further___builds_leaf_non_root_node(
        self, *uninteresting_mocks
    ):
        self.setup()
    ...
    @patch(
        "make_prg.recursion_tree.get_number_of_unique_ungapped_sequences",
        return_value=1,
    )
    def test___alignment_has_issues___too_few_unique_sequences_1(
        self, get_number_of_unique_ungapped_sequences_mock, *uninteresting_mocks
    ):
        self.setup()
        self.assertTrue(NodeFactory._alignment_has_issues(self.alignment))
        get_number_of_unique_ungapped_sequences_mock.assert_called_once_with(
            self.alignment
        )
    ...
    def test___alignment_has_issues___too_few_unique_sequences_2(
        self, get_number_of_unique_ungapped_sequences_mock, *uninteresting_mocks
    ):
        self.setup()
        self.assertTrue(NodeFactory._alignment_has_issues(self.alignment))
    ...
    @patch(
        "make_prg.recursion_tree.get_number_of_unique_gapped_sequences", return_value=4
    )
    def test___alignment_has_issues___alignment_has_ambiguity___ungapped_smaller_gapped(
        self,
        get_number_of_unique_gapped_sequences_mock,
        get_number_of_unique_ungapped_sequences_mock,
        *uninteresting_mocks,
    ):
        self.setup()
        self.assertTrue(NodeFactory._alignment_has_issues(self.alignment))
        get_number_of_unique_gapped_sequences_mock.assert_called_once_with(
            self.alignment
        )
        get_number_of_unique_ungapped_sequences_mock.assert_called_once_with(
            self.alignment
    ...
    def test___alignment_has_issues___alignment_has_ambiguity___ungapped_equals_gapped___no_issues(
        self,
        get_number_of_unique_gapped_sequences_mock,
        get_number_of_unique_ungapped_sequences_mock,
        *uninteresting_mocks,
    ...
    def test___alignment_has_issues___alignment_has_ambiguity___ungapped_larger_gapped___raises_AssertionError(
        self, *uninteresting_mocks
    ):
        self.setup()
        with self.assertRaises(AssertionError):
    ...
    def test___is_single_match_interval___single_match_interval(
        self, *uninteresting_mocks
    ):
    ...
    def test___is_single_match_interval___two_match_intervals(
        self, *uninteresting_mocks
    ):
    ...
    def test___is_single_match_interval___single_mismatch_interval(
        self, *uninteresting_mocks
    ):
    ...
    def test___is_single_match_interval___two_mismatch_intervals(
        self, *uninteresting_mocks
    ):
        interval_1 = Interval(IntervalType.NonMatch, 3, 10)
    ...
    def test___is_single_match_interval___single_match_interval_but_two_intervals(
        self, *uninteresting_mocks
    ):
        interval_1 = Interval(IntervalType.Match, 3, 10)
        interval_2 = Interval(IntervalType.NonMatch, 30, 100)
    ...
    def test___partition_alignment_into_interval_subalignments(
        ...
        self.assertEqual(
            len(expected_interval_subalignments), len(interval_subalignments)
        )
        self.assertTrue(
            all(
                map(equal_msas, expected_interval_subalignments, interval_subalignments)
            )
        )

    def test___infer_if_we_should_cluster_further___no_clustering(
        self, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=True)
        self.assertFalse(
            NodeFactory._infer_if_we_should_cluster_further(
                Mock(), clustering_result_mock, 3, 5
            )
        )

    def test___infer_if_we_should_cluster_further___max_nesting_reached___smaller(
        self, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=False)
        self.assertFalse(
            NodeFactory._infer_if_we_should_cluster_further(
                Mock(), clustering_result_mock, 4, 5
            )
        )

    def test___infer_if_we_should_cluster_further___max_nesting_reached___equal(
        self, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=False)
        self.assertFalse(
            NodeFactory._infer_if_we_should_cluster_further(
                Mock(), clustering_result_mock, 5, 5
            )
        )

    def test___infer_if_we_should_cluster_further___max_nesting_reached___larger(
        self, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=False)
        self.assertFalse(
            NodeFactory._infer_if_we_should_cluster_further(
                Mock(), clustering_result_mock, 6, 5
            )
        )

    @patch.object(
        NodeFactory, NodeFactory._alignment_has_issues.__name__, return_value=True
    )
    def test___infer_if_we_should_cluster_further___alignment_has_issues(
        self, alignment_has_issues_mock, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=False)
        alignment_mock = Mock()
        self.assertFalse(
            NodeFactory._infer_if_we_should_cluster_further(
                alignment_mock, clustering_result_mock, 3, 5
            )
        )
        alignment_has_issues_mock.assert_called_once_with(alignment_mock)

    @patch.object(
        NodeFactory, NodeFactory._alignment_has_issues.__name__, return_value=False
    )
    def test___infer_if_we_should_cluster_further___no_issues___ok_to_cluster(
        self, alignment_has_issues_mock, *uninteresting_mocks
    ):
        clustering_result_mock = Mock(no_clustering=False)
        alignment_mock = Mock()
    ...
    def test___get_subalignments_by_clustering(self, *uninteresting_mocks):
        alignment = make_alignment(
            ["AAAT", "C--C", "AATT", "GNGG", "CCCC", "TTTT", "AAAA"],
        ...
        expected = [
            make_alignment(["AAAT", "C--C", "TTTT", "AAAA"], ["s0", "s1", "s5", "s6"]),
            make_alignment(["GNGG"], ["s3"]),
            make_alignment(["AATT", "CCCC"], ["s2", "s4"]),
        ]
        actual = NodeFactory._get_subalignments_by_clustering(
    ...
    def test___get_sub_alignment_by_list_id___GivenOrderedIds_SubalignmentInSequenceOrder(
        self, *uninteresting_mocks
    ):
        self.setup()
